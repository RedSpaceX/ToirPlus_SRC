...
void CIniManager::WriteIniFile()
{
	...
	//m_IniFile->WriteString( (sz__App_Information),  "Combo_Key", m_szCombo_Key);
	//m_IniFile->WriteString( (sz__App_Information),  "LaneClear_Key", m_szLaneClear_Key);
	//m_IniFile->WriteString( (sz__App_Information),  "Harass_Key", m_szHarass_Key);
	//m_IniFile->WriteString( (sz__App_Information),  "LastHit_Key", m_szLastHit_Key);
	...
	//delete []m_szCombo_Key; m_szCombo_Key = 0;
	//delete []m_szLaneClear_Key; m_szLaneClear_Key = 0;
	//delete []m_szHarass_Key; m_szHarass_Key = 0;
	//delete []m_szLastHit_Key; m_szLastHit_Key = 0;
}
...
//----Combo-Key-------------------------
	if (m_szCombo_Key)
	{
		delete []m_szCombo_Key;
		m_szCombo_Key = 0;
	}
	m_szCombo_Key			= m_IniFile->ReadString( (sz__App_Information),  "Combo_Key", "SPACE");
	//SetCombo_Key(m_szCombo_Key) ;
//----LaneClear-Key-------------------------
	if (m_szLaneClear_Key)
	{
		delete []m_szLaneClear_Key;
		m_szLaneClear_Key = 0;
	}
	m_szLaneClear_Key			= m_IniFile->ReadString( (sz__App_Information),  "LaneClear_Key", "V");
	//SetLaneClear_Key(m_szLaneClear_Key) ;
//----Harass-Key-------------------------
	if (m_szHarass_Key)
	{
		delete []m_szHarass_Key;
		m_szHarass_Key = 0;
	}
	m_szHarass_Key			= m_IniFile->ReadString( (sz__App_Information),  "Harass_Key", "C");
	//SetHarass_Key(m_szHarass_Key) ;
//----LastHit-Key-------------------------
	if (m_szLastHit_Key)
	{
		delete []m_szLastHit_Key;
		m_szLastHit_Key = 0;
	}
	m_szLastHit_Key			= m_IniFile->ReadString( (sz__App_Information),  "LastHit_Key", "X");
	//OutputDebugStringA(m_szLastHit_Key);
	//SetLastHit_Key(m_szLastHit_Key) ;
...
//----Combo-Key-------------------------
	...
	m_szCombo_Key			= m_IniFile->ReadString( (sz__App_Information),  "Combo_Key", "");
//----LaneClear-Key-------------------------
	if (m_szLaneClear_Key)
	{
		delete []m_szLaneClear_Key;
	...
	m_szLaneClear_Key			= m_IniFile->ReadString( (sz__App_Information),  "LaneClear_Key", "");
//----Harass-Key-------------------------
	if (m_szHarass_Key)
	{
		delete []m_szHarass_Key;
	...
	m_szHarass_Key			= m_IniFile->ReadString( (sz__App_Information),  "Harass_Key", "");
//----LastHit-Key-------------------------
	if (m_szLastHit_Key)
	{
		delete []m_szLastHit_Key;
	...
	m_szLastHit_Key			= m_IniFile->ReadString( (sz__App_Information),  "LastHit_Key", "");
...
//------------------------------------------------------------------------------
char* CIniManager::GetCombo_Key() const { return m_szCombo_Key; }
void CIniManager::SetCombo_Key(const char* val) {
	if (m_szCombo_Key)
	{
		delete []m_szCombo_Key;
		m_szCombo_Key = 0;
	}
	m_szCombo_Key = new char[strlen(val) + 1];
	strcpy_s(m_szCombo_Key, MAX_PATH, val);
}

//------------------------------------------------------------------------------
char* CIniManager::GetLaneClear_Key() const { return m_szLaneClear_Key; }
void CIniManager::SetLaneClear_Key(const char* val) {
	if (m_szLaneClear_Key)
	{
		delete []m_szLaneClear_Key;
		m_szLaneClear_Key = 0;
	}
	m_szLaneClear_Key = new char[strlen(val) + 1];
	strcpy_s(m_szLaneClear_Key, MAX_PATH, val);
}

//------------------------------------------------------------------------------
char* CIniManager::GetHarass_Key() const { return m_szHarass_Key; }
void CIniManager::SetHarass_Key(const char* val) {
	if (m_szHarass_Key)
	{
		delete []m_szHarass_Key;
		m_szHarass_Key = 0;
	}
	m_szHarass_Key = new char[strlen(val) + 1];
	strcpy_s(m_szHarass_Key, MAX_PATH, val);
}

//------------------------------------------------------------------------------
char* CIniManager::GetLastHit_Key() const { return m_szLastHit_Key; }
void CIniManager::SetLastHit_Key(const char* val) {
	if (m_szLastHit_Key)
	{
		delete []m_szLastHit_Key;
		m_szLastHit_Key = 0;
	}
	m_szLastHit_Key = new char[strlen(val) + 1];
	strcpy_s(m_szLastHit_Key, MAX_PATH, val);
}
