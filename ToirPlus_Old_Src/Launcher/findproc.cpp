...
BOOL CALLBACK EWP_DirectInject(HWND hwnd, LPARAM lParam)
{
	...
	if(injdata.mode == 1)
	{
		...
		if(strncmp((LPCSTR)injdata.name, name, strlen(name) + 1) == 0)
		{
			...
			if(dwPid == 0)
			{
				//__oMsg("Could not get ProcessId from window handle (hwnd: 0x%p).",					hwnd);
			}
			...
			if(injdata.inject)
			{
				if(injdata.mm)
				{
					...
					{
						//__oMsg("Failed to map the PE file into the remote address space of a process (PID: %x)\n",							dwPid);
					}
				}
				else
				{
...
BOOL InjectEjectToProcessNameA(LPCSTR lpProcName, LPCSTR lpLibPath, LPVOID lpModule, BOOL inject, BOOL mm)
{
	...
	{
		//__oMsg("Could not get process snapshot.");
		return FALSE;
	}
			...
			if(!strncmp(lpProcName, pe32.szExeFile, strlen(lpProcName)))
			{
				...
				if(inject)
				{
					...
					else
					{
						...
						{
							//__oMsg("Injection failed. (PID: %x)", pe32.th32ProcessID);
						}
					}
				}
				else
				{
					if(lpModule)
					{
						...
						{
							//__oMsg("Ejection failed. (PID: %x)", pe32.th32ProcessID);
						}
					}
					else
					{
	...
	if(!bFound)
	{
		//__oMsg("Could not find process (%s).", lpProcName);
	}
