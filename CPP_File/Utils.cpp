...
extern bool bIsAntiGap;
extern bool nLUA_Combo;
extern bool nLUA_Harass;
extern bool nLUA_ClearLane;
...
extern float Tick_doQ, Tick_doW, Tick_doE, Tick_doR, Tick_doOtherSkill, MaxSkillDelay_1, MaxSkillDelay_2;

//==================================================================================//
bool Get_Exit()
{
	extern bool lua_exit;
	return lua_exit;
}
...
extern void OnIssueOrder_Evade(KNpc* hero, GameObjectIssueOrderEventArgs* args);
float LastProcess = 0;

float LastCall_MoveToFunc = 0;
//======================================================================================================
void MOVE_TO(float &posX, float &posY, bool triggerEvent)
{
	//// MOVE_TO() chan. delay ben trong, NormalAttack() chan. delay ben ngoai`, 
	
	if (CAN_CALL_MOVETO_FUNC() == false) return;

	if(triggerEvent)
	{
		if(ArgsMove == 0) return;

		Vec3D pos(posX, GetPlayer() == 0 ? 0.0f : GetPlayer()->CurrentPos.y, posY);

		ArgsMove->Order = 2;
		ArgsMove->TargetPosition = pos;
		ArgsMove->Target = 0;
		ArgsMove->AttackMove = false;
		ArgsMove->Process = true;

		//--HamDetour o day : ...todo
		OnIssueOrder_Evade(0, ArgsMove);

		if(ArgsMove->Process == false)
		{
			//__oMsg("RETURN-CHAN.MOVE_TO------- 111111111");
			//LastProcess = (float)GetTickCount();
			return;
		}

		////---- co the ko can doan nay :
		//if(ArgsMove->Process == true && (float)GetTickCount() - LastProcess < 125)
		//{
		//	//__oMsg("RETURN-CHAN.MOVE_TO------- 222222222");
		//	return;
		//}
	}

	//__oMsg("HETTTT-CHAN.MOVE_TO-------: triger: %d", triggerEvent);
...
retnHere:
	...
	*(DWORD*)BASE_ANTI_ISSUEORDER = 0x13;
	...
	//__oMsg_2xxx("------------MOVING: %f", 1000*(GetTimeGame() - LastCall_MoveToFunc));
	LastCall_MoveToFunc = GetTimeGame();
}	
...
//--------------------------------------------------------------------------------
float rClick_attack = 0;
void RCLICK_ATTACK_NPC_TESTING(KNpc* unit) 
{
	if ((float)GetTickCount() - rClick_attack < 699) return;
	...
	KPos vScreen, pPosIn;
	pPosIn.x = unit->CurrentPos.x;
	pPosIn.y = unit->CurrentPos.y;
	pPosIn.z = unit->CurrentPos.z;
	WorldPos_to_ScreenPos(&pPosIn, &vScreen);
	int nPosXScreen = (int)vScreen.x;
	int nPosYScreen = (int)vScreen.y;
	__asm
	...
	rClick_attack = (float)GetTickCount();
}
...
void Block_RightClick_FromUser()
{
	...
	if (pPlayer->nBLOCK_MOVE != 1) pPlayer->nBLOCK_MOVE = 1;
}
...
void UnBlock_RightClick_FromUser()
{
	...
	if (pPlayer->nBLOCK_MOVE != 0) pPlayer->nBLOCK_MOVE = 0;
}

//==================================================================================//
void SEND_TEXT_SYSTEM(char *szTextMsg)
{
	if(pLang->isNoTextChat) return;
...
{
	HWND g_hWinAPP = FindWindow(_T("RiotWindowClass"), NULL); // Game
...
HMODULE MyGetModuleHandle(const char* name)
{
	...
	////------- T+ Public 4China ------------------------------------------:
	//DWORD mod_LOLBase = 0;
	//char sz__LOLBase[] = {76,79,76,66,97,115,101,46,100,108,108,0};	// "LOLBase.dll"
	//mod_LOLBase = (DWORD)GetModuleHandleA(sz__LOLBase);
	//if (mod_LOLBase == 0)
	//{
	//	if (IsGameReady())
	//	{	
	//		//Your region is not China
	//		char szTemp[] = {89,111,117,114,32,114,101,103,105,111,110,32,105,115,32,110,111,116,32,67,104,105,110,97,0};
	...
	//	//////is_China = false;
	//	hModule = GetModuleHandle(name) + g_Address;
...
//==================================================================================//
//==================================================================================//
extern KLoadScripts g_LoadScript[MAX_SCRIPT_RUN];
extern bool bIsLuaLoaded;
extern callback_OnBeforeAttack callback_BeforeAttack;

void NormalAttack(KNpc *pNpc, bool triggerEvent)
{
	//// normalAttack chan. delay ben ngoai`, MOVE_TO chan. delay ben trong

	if(triggerEvent)
	{
		if (ArgsMove == 0) return;
		if (!pNpc) return;
		//if (pNpc->nIndex < 0x40000000) return;

		ArgsMove->Order			 = 3;
		ArgsMove->TargetPosition = pNpc->currentPos();
		ArgsMove->Target		 = pNpc;
		ArgsMove->AttackMove	 = false;
		ArgsMove->Process		 = true;
		...
		if(ArgsMove->Process == false)
		{
			//__oMsg("RETURN-CHAN.AUTO_ATTACKKK------- 111111111");
			//LastProcess = (float)GetTickCount();
			return;
		}

		////---- co the ko can doan nay :
		//if(ArgsMove->Process == true && (float)GetTickCount() - LastProcess < 200)
		//{
		//	__oMsg("RETURN-CHAN.MOVE_TO------- 222222222");
		//	return;
		//}
	...
	if(callback_BeforeAttack) callback_BeforeAttack(pNpc);
...
{
	if (!G_Module || !myTeamId) return false;

	DWORD nTemUnit    = BASE_AttackableUnit - TEMP_ENTRY_POINT + G_Module;
	DWORD nTemUnitMax = BASE_AttackableUnit_MAX - TEMP_ENTRY_POINT + G_Module;
	DWORD dw1 = *(DWORD*)nTemUnit;
	DWORD dw2 = *(DWORD*)nTemUnitMax;
	...
	for (int i = 0; i < nCountUnit; ++i)
	{
		...
		if (pAllyMinion->nTeam_ID != myTeamId)				   continue;
		if (!IS_MINION(pAllyMinion))						   continue;
		if (!IsValidTarget(pAllyMinion, nRange, false))		   continue;	
		if (!CheckName(sz__Minion, pAllyMinion->GetNameNpc())) continue;
...
bool Is_EnemyMinion_Around_NPC(KNpc* pNpc, float nRange)
{
	...
	for (int i = 0; i < nCountUnit; ++i)
	{
		...
		if (!IS_MINION(pEnemyMinion))							continue;
		if (!IsValidTarget(pEnemyMinion, nRange, true))			continue;	//co check team trong roi
		if (!CheckName(sz__Minion, pEnemyMinion->GetNameNpc())) continue;
...
int CountEnemyMinionInRange(float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(!IS_MINION(enemyMinion)) continue;
...
int Count_EnemyPlayer_Around_NPC(KNpc* pNpc, float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(Distance(pNpc, enemy) > nRange) continue;
		countEnemy += 1;
	}
	return countEnemy;
}
...
int Count_EnemyMinion_Around_NPC(KNpc* pNpc, float nRange)
{
	...
	for (int i = 0; i < nCountUnit; ++i)
	{
		...
		if (!IS_MINION(pEnemyMinion))			  continue;
		if (!IsValidTarget(pEnemyMinion, nRange)) continue;
		if (!CheckName(sz__Minion, pEnemyMinion->GetNameNpc())) continue;
...
int Count_AllyPlayer_Around_NPC(KNpc* pNpc, float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(Distance(pNpc, allyPlayer) > nRange)      continue;
		jCount += 1;
	}
	return jCount;
}
...
int Count_AllyMinion_Around_NPC(KNpc* pNpc, float nRange)
{
	...
	for (int i = 0; i < nCountUnit; ++i)
	{
		...
		if (pAllyMinion->nTeam_ID != myTeamId) continue;
		if (IS_DEAD(pAllyMinion))			   continue;

		if (!IS_MINION(pAllyMinion))		   continue;
		if (!CheckName(sz__Minion, pAllyMinion->GetNameNpc())) continue;

		if (Distance(pNpc, pAllyMinion, true) > nRange * nRange) continue;
...
KNpc* GET_VISION_WARD(float nRange, bool isMat_Tim)
{
	...
	//	int isDie = IS_DEAD(pVisionWard) == true ? 1 : 0;
...
KNpc* SearchNearestAllyTurret(float nRange)
{
	...
	for (int i = 0; i < countTurret; ++i)
	{
		...
		if (!IS_TURRET(pTurret))					continue;
		if (pTurret->nTeam_ID != myTeamId)			continue;
		if (!IsValidTarget(pTurret, nRange, false)) continue;

		pTru[jCount].pEnemy = pTurret;
		pTru[jCount].fDistance = Distance(pPlayer, pTurret);
		jCount += 1;
	}
...
KNpc* SearchNearestAllyPlayer(float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		int isDie = IS_DEAD(pAllyPlayer) == true ? 1 : 0;
		if (pAllyPlayer->IsEnemy(pPlayer->nTeam_ID) || isDie == 1 || pAllyPlayer->nHP <= 0 || !IS_HERO(pAllyPlayer))
			continue;
...
KNpc* SearchNearestNPC(float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		//	//__oMsg("1--nID_EnemySelected: %x", nID_EnemySelected);
		//	if(nID_EnemySelected == pEnemyPlayer->nID)
		//	{
		//		//__oMsg("2--nID_EnemySelected: %x", nID_EnemySelected);
		//		return pEnemyPlayer;
		//	}
		//}
...
KNpc* SearchNearestNPC_Minion(float nRange)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if (dw3 == 0) continue;
		KNpc *pEnemyMinion = (KNpc*)dw3;
		int isDie = IS_DEAD(pEnemyMinion) == true ? 1 : 0;
		if (!pEnemyMinion->IsEnemy(pPlayer->nTeam_ID) || isDie == 1 || pEnemyMinion->nHP <= 0. || !IS_MINION(pEnemyMinion))
			continue;
...
{
	return GetTargetSelector(GetPlayer(), nRange, DmgPhysical);
//--------------------
...
float GetDamageReceive(KNpc* pPlayer, KNpc* pEnemy)
{
	...
	//-- Yorick -------------:
		extern bool isYorick;
		if (isYorick)
		{
			//[5236] [Type = 2][c3ead00]Buff Name:  yorickqbuff
			if (GET_BUFF(pPlayer, "yorickqbuff"))
			{
				KSkill*   pSkill = (KSkill*) pPlayer->_pSkill[_Q];
				if(!pSkill) return 0;
				int levelSpell  = (BYTE)pSkill->nLevel > 5 ? 5 : (BYTE)pSkill->nLevel;
				float dmgSkill[6] = {0, 30, 55, 80, 105, 130};
				float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // co tinh cong ca STVL co ban

				nDmgOfPlayer += dmgSkill[levelSpell] + 0.4f * stvl;
			}
		}

	//-- Corki -------------:
		extern bool isCorki;
		if (CheckName(pPlayer->GetNameHero(), "Corki"))
		{
			nDamageReceive = calDamage(pEnemy, 0.8f * nDmgOfPlayer, DmgMagical) + calDamage(pEnemy, 0.2f * nDmgOfPlayer, DmgPhysical);
		}

	//-- Shyvana -------------:
		extern bool isShyvana;
		if (isShyvana)
		{
			if (CanCastSpell2(_Q) == 8)
			{
				KSkill*   pSkill = (KSkill*) pPlayer->_pSkill[_Q];
				if(!pSkill) return 0;
				int levelSpell  = (BYTE)pSkill->nLevel > 5 ? 5 : (BYTE)pSkill->nLevel;
				float dmgSkill[6] = {0, 20, 35, 50, 65, 80};
				float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // co tinh cong ca STVL co ban

				nDmgOfPlayer += dmgSkill[levelSpell]/100 * stvl;
			}
		}
		
	//-- Draven -------------:
		extern bool isDraven;
		if (isDraven)
		{
	...
	//-- MISS-FORTUNE ------:
		extern bool isMissF;
		if (isMissF && IS_MINION(pEnemy))// MissFortune-tinh voi linh thoi
		{
			...
			if(pEnemy->nIndex != MissFLastAtkId)
			{
				...
				//if(IS_MINION(pEnemy)) bonus = bonus/2;
	...
	//////-- PANTHEON -------------:
	////	extern bool isPantheon;
	////	if (isPantheon)
	////	{
	////		if (GET_BUFF(pPlayer, sz__PantheonEPassive))	// PantheonEPassive: noi tai E cua Pantheon
	////		{
	...
	//-- ZED -------------:
		...
		if (isZed)
		{
			...
			if (nPercent <= 50)
			{
				nDmgOfPlayer += 0.06f*pEnemy->nMaxHP;
			}
		}
	...
	//-- CAITLYN -------------:
		extern bool isCaitlyn;
		if (isCaitlyn)
		{
			if(GET_BUFF(pPlayer,  (sz__caitlynheadshot)))
			{
				float dmgHeadShot = 0.5f;		//150% - 175% - 200%
				if(IS_MINION(pEnemy))	dmgHeadShot = 1.0f;

				nDmgOfPlayer += dmgHeadShot*nDmgOfPlayer;
				
			}
		}

	//-- LUCIAN -------------:
		extern bool isLucian;
		if (isLucian)
		{
			if(GET_BUFF(pPlayer,  (sz__LucianPassiveBuff)))
			{
				if(IS_MINION(pEnemy))	nDmgOfPlayer += nDmgOfPlayer; // ban 2 lan, chi tinh linh, tuong thi tinh theo cong thuc khac(ko can thiet phai tinh)
			}
		}
	...
	//-- Yasuo -------------:
		...
		if (isYasuo)
		{
			if (abs(pPlayer->nCritChance - 1) < FLT_EPSILON)
			nDmgOfPlayer = (float)0.9 * nDmgOfPlayer;
		}
	...
	//-- Jhin -------------:
		...
		if (isJhin)
		{
			...
			if(GET_BUFF(pPlayer,  (sz__jhinpassiveattackbuff)))
			{
				...
				nDmgOfPlayer += (float)0.2*pPlayer->nFlat_PhysicalDamageMod + nPercent_HP_Missed*(pEnemy->nMaxHP - pEnemy->nHP); 
			}
			...
			if ((BYTE)pPlayer->nLevel <= 9)
			{
				nPercentBonusAD = 0.03f + 0.01f * (BYTE)pPlayer->nLevel;
			}
			else if ((BYTE)pPlayer->nLevel <= 11)
			{
				nPercentBonusAD = 0.12f + 0.02f * ((BYTE)pPlayer->nLevel - 9);
			}
			else if ((BYTE)pPlayer->nLevel <= 18)
			{
				nPercentBonusAD = 0.16f + 0.04f * ((BYTE)pPlayer->nLevel - 11);
			}
	...
	//-- NASUS -------------:
		...
		if (isNasus)
		{
			if(GET_BUFF(pPlayer,  (sz__NasusQ)) || CanCastSpell2(SKILL_Q) == 8)
			{
				...
				if(pSkillQ)
				{
					if(pSkillQ->nLevel != 0)
					{
						float dmgSkill[6] = {0, 30, 50, 70, 90, 110};	
						KBuffs*  pBuff_NasusQStacks = GET_BUFF(pPlayer,  (sz__NasusQStacks));
						int nCount_QStacks = 0;
						if (pBuff_NasusQStacks) nCount_QStacks = pBuff_NasusQStacks->nCount;

						nDmgOfPlayer  += 10 + 20*pSkillQ->nLevel /*+ pSkillQ->nAdd_Dmg*/ + nCount_QStacks; //BONUS PHYSICAL DAMAGE: 30 / 50 / 70 / 90 / 110 (+ Siphoning Strike stacks)
					}
				}
			}
		}
		...
		if (isShaco)
		{
			if(GET_BUFF(pPlayer, "Deceive"))
			{
				...
				if(pSkillQ)
				{
					if(pSkillQ->nLevel != 0)
					{	
						float dmgSkill[6] = {0, 10, 20, 30, 40, 50};
						float smpt         = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod; // AP
						float stvl   = pPlayer->nFlat_PhysicalDamageMod; // Bonus AD

						nDmgOfPlayer  += dmgSkill[pSkillQ->nLevel] + 0.4f * stvl + 0.3f * smpt;
					}
				}
			}
		}
	...
	//-- GAREN -------------:
		...
		if (isGaren)
		{
			if (CanCastSpell2(SKILL_Q) == 8)
			{
				KSkill*   pSkill = (KSkill*) pPlayer->_pSkill[_Q];
				if(!pSkill) return 0;
				int levelSpell  = (BYTE)pSkill->nLevel > 5 ? 5 : (BYTE)pSkill->nLevel;
				float dmgSkill[6] = {0, 30, 60, 90, 120, 150};
				float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // total damge
	...
	//-- Vi -------------:
		...
		if (isVi)
		{
			KBuffs* pBuff = GET_BUFF(pEnemy,  "viwproc");
			if(pBuff) 
			{
				int stacks = (pBuff->nEnd_Count - pBuff->nStart_Count)/8;
				if (stacks == 2)
				{
					KSkill*   pSkillW = (KSkill*) pPlayer->_pSkill[SKILL_W];
					if (pSkillW && 0 != pSkillW->nLevel)
					{
						float stvl   = pPlayer->nFlat_PhysicalDamageMod; // Bonus AD
						float dmgSkill[6] = {0, 4, 5.5f, 7, 8.5f, 10};
						nDmgOfPlayer  += (dmgSkill[pSkillW->nLevel]/100 + (0.01f * stvl/35))*pEnemy->nMaxHP;
					}
				}
			}
		}
		...
		if (isCamille)
		{
			...
			if(pSkillQ)
			{
				...
				if(pKSkill_Q)
				{
					...
					{
						if(GET_BUFF(pPlayer,  (sz__CamilleQ)) || CanCastSpell2(SKILL_Q) == 8)
						{
							if(pSkillQ->nLevel != 0)
							{
					...
					else														   // Q2-xin
					{
						if(GET_BUFF(pPlayer,  (sz__camilleqprimingcomplete)))
						{
							if(pSkillQ->nLevel != 0)
							{
								...
								int levelSpell  = (BYTE)pSkillQ->nLevel > 5 ? 5 : (BYTE)pSkillQ->nLevel;

								float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // total damge
								float dmgSkill[6] = {0, 40, 50, 60, 70, 80};
								float dmgSkillPer[19] = {0, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 100, 100};
								float DamBonus = (dmgSkill[levelSpell] / 100) * stvl + stvl;	
								float DamTrue = (dmgSkillPer[(BYTE)pPlayer->nLevel] / 100) * DamBonus;
								nDmgOfPlayer = DamTrue;
							}
						}
					}
				}
	...
	//-- MALPHITE -------------:
		extern bool isMalphite;
		if (isMalphite && GET_BUFF(pPlayer,  (sz__MalphiteCleave)))
	...
	//-- MASTER YI -------------:
		extern bool isMasterYi;
		if (isMasterYi && GET_BUFF(pPlayer,  (sz__wujustylevisual)) )
		...
		if(!IS_HERO(pEnemy)) nXuyenGiap = 0;
		...
		if (isViktor)
		{
			if(GET_BUFF(pPlayer,  (sz__ViktorPowerTransferReturn)))
			{
				...
				if(pSkillQ)
				{
					if(pSkillQ->nLevel != 0)
					{
						//20 / 50 / 80 / 110 / 140 (+ 100% AD) (+ 50% AP)
						float dmgDischarge[6]= {0, 20, 50, 80, 110, 140};
						nDamageReceive_AP += dmgDischarge[pSkillQ->nLevel] + (float)0.5*(pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod) // dame nay tinh theo dame phep, co tang theo cap...
											 + 1.0f*(pPlayer->nFlat_PhysicalDamageMod + pPlayer->nBaseAttackDamage);
						if (pEnemy->nSpellBlock >= 0)
						{
							nDamageReceive_AP = nDamageReceive_AP * (100/(100 + pEnemy->nSpellBlock));
						}
	...
	//--Warwick--------
		extern bool isWarwick;
		if (isWarwick)
		{
			nDamageReceive_AP += (BYTE)pPlayer->nLevel * 2.0f + 8.0f;
			//nDamageReceive_AP = damageAP;
			if (pEnemy->nSpellBlock >= 0)
			{
				nDamageReceive_AP = nDamageReceive_AP * (100/(100 + pEnemy->nSpellBlock));
			}
	...
	//-- VARUS ------
		...
		if (isVarus)
		{
			...
			if(pSkillW)
			{
				if(pSkillW->nLevel != 0)
				{
					float dmgSkill[6] = {0, 7, 10.5f, 14, 17.5f, 21};
					float smpt = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod;
					nDamageReceive_AP = calDamage(pEnemy, dmgSkill[pSkillW->nLevel] + 0.25f * smpt, DmgMagical);
	...
	//-- LUX ------
		extern bool isLux;
		if (isLux && GET_BUFF(pEnemy,  (sz__LuxIlluminatingFraulein))) // enemy bi hieu ung anh sang cua Lux
	...
	//-- KENNEN ------
		extern bool isKennen;
		if (isKennen && GET_BUFF(pPlayer,  (sz__kennendoublestrikelive)))
		{//MAGIC DAMAGE: 40 / 50 / 60 / 70 / 80% AD
			KSkill*   pSkillW = (KSkill*) pPlayer->_pSkill[SKILL_W];
			if (pSkillW && 0 != pSkillW->nLevel)
			{
				float dmgSkill[6] = {0, 10, 20, 30, 40, 50};
				float dmgPer[6] = {0, 60, 70, 80, 90, 100};
				float smpt         = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod; // AP
				float stvl   = pPlayer->nFlat_PhysicalDamageMod; // Bonus AD

				nDamageReceive_AP = calDamage(pEnemy, dmgSkill[pSkillW->nLevel] + dmgPer[pSkillW->nLevel]/100 * stvl + 0.2f * smpt, DmgMagical);
			}
		}

	//-- Galio ------
		extern bool isGalio;
		if (isGalio && GET_BUFF(pPlayer, "galiopassivebuff"))
		{
			int levelSpell  = pPlayer->nLevel;
			float smpt         = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod; // AP
			float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // co tinh cong ca STVL co ban

			nDamageReceive_AP = calDamage(pEnemy, ((levelSpell * 4) + 8)  + 0.5f * smpt + 0.4f * pPlayer->nSpellBlock_CongThem, DmgMagical);
		}

	//-- KHAZIX ------
		extern bool isKhazix;
		if (isKhazix && IS_HERO(pEnemy))
		{
	...
	//-- JAX ------ 
		...
		if (isJax)
		{
			if(GET_BUFF(pPlayer,  (sz__JaxEmpowerTwo)) || CanCastSpell2(SKILL_W) == 8)
			{
				KSkill*   pSkillW = (KSkill*) pPlayer->_pSkill[SKILL_W];
				if(pSkillW)
				{
	...
	//-- SONA ------ 
		extern bool isSona;
		if (isSona && GET_BUFF(pPlayer,  (sz__sonapassiveattack)))
		{
	...
	//-- LULU ------ 
		...
		if (isLulu)
		{
			KNpc* pPix =  Get_Lulu_Pix_AroundNPC(pPlayer, 2000);
			if(pPix)
			{
				int nMinion_ChanDuong_Pix = NPC_INLINE_SKILL(pEnemy, &pPix->CurrentPos, &pEnemy->CurrentPos, 70.0, 650.0);
				if(!nMinion_ChanDuong_Pix)
				{
					//float nDame_NoiTai = 3 + 4*(int)((BYTE)pPlayer->nLevel/2.0 - 0.4); // 3 tia
					float nDame_NoiTai = (float)(3 + 2*(BYTE)pPlayer->nLevel);					 // 3 tia
					if (pEnemy->nSpellBlock >= 0)
					{
						nDamageReceive_AP = (nDamageReceive_AP + nDame_NoiTai + (float)0.05*(pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod)) * (100/(100 + pEnemy->nSpellBlock));
					}
					else
					{
						nDamageReceive_AP = (nDamageReceive_AP + nDame_NoiTai + (float)0.05*(pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod)) * (2-100/(100 - pEnemy->nSpellBlock));
					}
					nDamageReceive_AP += nDamageReceive_AP; // lay 2 lan thoi, lan thu3 toc do cham
				}
			}
		}
	...
	//-- DIANA ------
		...
		if (isDiana)
		{
			KBuffs* pBuff = GET_BUFF(pPlayer,  (sz__dianapassivemarker));
			if(pBuff)
			{
				int nCount = (pBuff->nEnd_Count - pBuff->nStart_Count)/8;
				if(nCount >= 2)
				{
					/*float nDam_NT = DIANA_GetDamage_NoiTai(pEnemy);
					nDamageReceive_AP += nDam_NT;*/

					float dmgSkill[19] = {0, 20, 25, 30, 35, 40, 50, 60, 70, 80, 90, 105, 120, 135, 155, 175, 200, 225, 250};
					float smpt         = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod; // AP
					nDamageReceive_AP += calDamage(pEnemy, dmgSkill[(BYTE)pPlayer->nLevel] + 0.8f * smpt, DmgMagical);
				}
			}
		}

	//-- CHO"GATH ------
		extern bool isChogath;
		if (isChogath)
		{
			if(GET_BUFF(pPlayer,  (sz__VorpalSpikes)))
			{
				float nDam_E = dmgE_Chogath(pPlayer, pEnemy);
				nDamageReceive_AP += nDam_E;
			}
		}
	//============================
		...
		if (isVayne)
		{
			...
			if(pBuff)
			{
				...
				if(nCount >= 2)
				{
					...
					if(IS_MINION(pEnemy) && nDam_Chuan > 200) nDam_Chuan = 200;
	...
	//-- GNAR ------------:
		...
		if (isGnar)
		{
			KSkill*   pSkill = (KSkill*) pPlayer->_pSkill[_W];
			if(!pSkill) return 0;
			int levelSpell  = (BYTE)pSkill->nLevel > 5 ? 5 : (BYTE)pSkill->nLevel;
			float stvl   = pPlayer->nBaseAttackDamage + pPlayer->nFlat_PhysicalDamageMod; // co tinh cong ca STVL co ban
			float smpt         = pPlayer->nFlat_MagicDamageMod + pPlayer->nFlat_MagicDamageMod*pPlayer->nPercent_MagicDamageMod; // AP
			float dmgSkill[6] = {0, 10, 20, 30, 40, 50};
			float dmgPer[6] = {0, 6, 8, 10, 12, 14};
			KBuffs* pBuff = GET_BUFF(pEnemy,  "gnarwproc");
			if(pBuff) 
			{
				int stacks = (pBuff->nEnd_Count - pBuff->nStart_Count)/8;
				if (stacks == 2)
				{
					nDmgOfPlayer  += dmgSkill[levelSpell] + dmgPer[levelSpell]/100 * pEnemy->nMaxHP + smpt;//calDamage(pEnemy, dmgSkill[levelSpell] + dmgPer[levelSpell]/100 * pEnemy->nMaxHP + smpt, DmgMagical);
				}
			}
		}
	...
	//-- MASTER YI ------
		extern bool isMasterYi;
		if (isMasterYi && GET_BUFF(pPlayer,  (sz__wujustylesuperchargedvisual)) )
	...
	//{
	//	nDamageReceive = nDmgOfPlayer;
	//	SEND_TEXT_SYSTEM("ERROR : GET AA DMG");
	//}
...
{
	return GetTargetSelector(GetPlayer(), nRange, DmgMagical);
//--------------------
...
bool IsCanSee(KNpc *pEnemy)
{
	...
	if(pEnemy && pEnemy->nIsTargetableToTeam == 4 && (IS_NEXUS(pEnemy) || IS_INHIBITOR(pEnemy))) return true;
	DWORD bCanSee = 0;
	DWORD hModule	= (DWORD)MyGetModuleHandle( (sz__League_of_Legends_exe));
...
bool IS_ADC(KNpc *pNpc)
{
	...
	if(IS_DEAD(pNpc) == true) return nIs_ADC;
...
extern float Riven_Time_DoSkill;
extern float nVayne_TimeDoSkill_E;
...
bool IsGameReady()
{
	//__try
	{
		...
		//__oMsg("(BYTE)tmp->nGiaTri : %d", (BYTE)tmp->nGiaTri);
		if((BYTE)tmp->nGiaTri < 2) return false;
	}
	//__except(EXCEPTION_EXECUTE_HANDLER)
	//{
	//	SEND_TEXT_SYSTEM("Err: IsGameReady");
	//	return false;
	//}
	
	static UINT nTime2CheckGameReady = 0;
	nTime2CheckGameReady += 30;
	
	if(nTime2CheckGameReady < 1000)
	{
		//__oMsg("Game time:%f", GetTimeGame());
		return false;
	}
	...
	//if (nTimeGame < 1) return false;

	//if (g_Exit == true)
	//{
	//	
	//	Retour_OnProcessSpell();
	//	Retour_EndScene();
	//	Retour_OnCreateObj();
	//	Retour_OnDeleteObj();
	//	Retour_OnUpdateBuff();
	//	Retour_OnRemoveBuff();
	//	Retour_OnPlayAnimation();
	//	Retour_OnDoCast();
	//	Retour_OnIssueOrder();
	//	Retour_OnNewPath();
	//	//Retour_OnStopCast();
	//	//Retour_OnProcessPacket();
	//	return false;
	//}
...
bool DodgeMissileLine(KPos* startPoint,KPos* endPoint, float radius, float range, KBuffer_Spell* pBufferSpell, float timeFlying, bool flag, fPtrRESET_DATA_SPELL fPtrRESET)
{
	...
	if (/*SLMHTClientData::GetInstance()->GetCBEvadeLine()*/ pLang->isEvadeNormalSkill== false)   // Nut DODGE LINE Ngoai giao dien 
	{
		if (/*SLMHTClientData::GetInstance()->GetCBEvadeDangerous()*/pLang->isEvadeDangerousSkillOnly == false)
		{
			return FALSE;
		}
		else
		{
			if (pBufferSpell->nIsDangerous == false)
			{
				return FALSE;
			}
		}
...
//--- moi them ---------------------------------------
	...
	extern bool isSivir, Sivir_autoEallSkill;
	if(isSivir && Sivir_autoEallSkill && CanCast(_E)) return false;
...
bool WINAPI DodgeCircle(KPos* pos1, float radius, KBuffer_Spell* pBufferSpell, float timeFlying, fPtrRESET_DATA_SPELL fPtrRESET, bool flagFlash)
{
	...
	if (/*SLMHTClientData::GetInstance()->GetCBEvadeCircle()*/pLang->isEvadeCircleSkill == false)   // Nut DODGE CIRCLE Ngoai giao dien 
	{
		if (/*SLMHTClientData::GetInstance()->GetCBEvadeDangerous()*/pLang->isEvadeDangerousSkillOnly == false)
		{
			return FALSE;
...
DODGE:
	//----------------------------
	if (flagFlash == true && pLang->isFlasfToEvade)
	{
		for (int i = 0; i < 20; ++i)
		{
			KSkill *pSkill = (KSkill *)g_pPlayer->_pSkill[i];
			if(!pSkill) continue;
			...
			{
				////__oMsg("abc to -----------------%d", (BYTE)CanCastSpell(pKSkill_2GetName));
				if ((BYTE)CanCastSpell2(i) == 0)
				{
					////__oMsg("move to -----------------%d", CanCastSpell(pKSkill_2GetName));
					MOVE_TO(dodge_x,dodge_z);
				}
				else
				{
					////__oMsg("do flash -----------------%d", CanCastSpell(pKSkill_2GetName));
					DO_FLASH(new KPos(dodge_x, 0., dodge_z));
				}
			}
		}
	}
...
int COUNT_NPC_INCIRCLE(KPos *posTam, KBuffer_Spell *pBufferSpell)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if (!pNpc->IsAlly(pPlayer->nTeam_ID) || isDie == 1 || pNpc->nHP <= 0. || !IS_MINION(pNpc))
			continue;
...
//--- moi them ---------------------------------------
	...
	//----------------------------------------------------------------
	if (pBufferSpell->nDangerValue == 5 && pLang->isFlasfToEvade)
	{
		if(DO_FLASH(new KPos(EVADE_x, 0., EVADE_z)) == false) 
		{
			if(IsShouldCircle(posTam, pBufferSpell) == false) return FALSE;
			BlockMove();
			MOVE_TO(EVADE_x,EVADE_z);
		}
	}
	...
	{
		if(IsShouldCircle(posTam, pBufferSpell) == false)
		{
			//__oMsg("DeoNeDc chu sao nua");
			return FALSE;
		}
		BlockMove();
		MOVE_TO(EVADE_x,EVADE_z);
...
START_EVADE:
	...
	//----------------------------
	if(pBufferSpell->nDangerValue == 5 && pLang->isFlasfToEvade)  DO_FLASH(&dodge);  //TOC BIEN NEU CHIEU DAC BIET NGUY HIEM
	else        
	{
		if (IsShouldDodge(startPoint, endPoint, pBufferSpell) == false) return FALSE;
		BlockMove();
		MOVE_TO(dodge.x, dodge.z);
	}
	return TRUE;
}
//==================================================================================//
...
//--- moi them ---------------------------------------
	...
	{
		if (IsShouldDodge(startPoint, endPoint, pBufferSpell) == false)
		{
			//__oMsg("KO-ne-Do-XXXXXXXXXX");
			return FALSE;
		}
		BlockMove();
		MOVE_TO(dodge.x, dodge.z);
...
START_EVADE:
	...
	else        
	{
		...
		BlockMove();
		MOVE_TO(dodge.x, dodge.z);
		SET_IsEvaded_BUFFER_SPELL_SPECIAL(pBufferSpell->index);
	}
	return TRUE;
}
//==================================================================================//
...
//--- moi them ---------------------------------------
	...
	if(pBufferSpell->nDangerValue == 5 && pLang->isFlasfToEvade)  DO_FLASH(&evade);  //TOC BIEN NEU CHIEU DAC BIET NGUY HIEM
	else        
	{
		//if (IsShouldDodge_45Do(startPoint, endPoint, pBufferSpell, nTienLui) == false) return FALSE;
		BlockMove();
		MOVE_TO(evade.x, evade.z);
	}
	return TRUE;
}
...
void SET_CONDITION_BUFF_2(KNpc* unit, char * animation)
{
	KNpc *pPlayer = GetPlayer();
	if(!pPlayer)				   return;
	if(IS_DEAD(pPlayer) == true) return;

	if(unit && IS_HERO(unit) && unit->IsEnemy(pPlayer->nTeam_ID) && IS_DEAD(unit) == false && 
...
//	if(!pPlayer)				   return;
//	if(IS_DEAD(pPlayer) == true) return;
//	if(!unit || !IS_HERO(unit) || !unit->IsEnemy(pPlayer->nTeam_ID) || IS_DEAD(unit) == true || !pCast) return;
//	
...
//							int isDie = IS_DEAD(pOri_Sphere) == true ? 1 : 0;
...
void SET_BASE_TIME_SPELL()
{
	...
	//				
	//				DWORD movNote = GET_PMOVE_NEW(pEnemyPlayer);
...
//	{
//		// kiem tra xem co hero nao ra skill
//		if (!g_BaseTimeSpell[i].isCasting) continue;
//		KBuffer_Spell pBuff = g_BaseTimeSpell[i];
//		float fDistance = Distance_2D(pPlayer->CurrentPos, pBuff.SourcePos);
//		
////=11===SEJUANI==========================================================================================================
//		if (CheckName(pBuff.szHeroName,  (sz__Sejuani)))
//		{
//			CDodge::GetInstance()->DodgeSEJUANI_BASETIME(pBuff);
//		}
////==2===GRAGAS=====================================================================================================
//		else if (CheckName(pBuff.szHeroName,  (sz__Gragas)))
//		{
//			CDodge::GetInstance()->DodgeGRAGAS_BASETIME(pBuff);
//		}
////==9===AATROX==========================================================================================================
//		else if (CheckName(pBuff.szHeroName,  (sz__Aatrox)))
//		{
//			CDodge::GetInstance()->DodgeAATROX_BASETIME(pBuff);
//		}
////=35===DARIUS==========================================================================================================
//		else if (CheckName(pBuff.szHeroName,  (sz__Darius)))
//		{
//			CDodge::GetInstance()->DodgeDARIUS_BASETIME(pBuff);
//		}
////=41===MALPHITE==========================================================================================================
//		else if (CheckName(pBuff.szHeroName,  (sz__Malphite)))
//		{
//			CDodge::GetInstance()->DodgeMALPHITE_BASETIME(pBuff);
//		}
////107===TRYNDAMERE==========================================================================================================
//		else if (CheckName(pBuff.szHeroName,  (sz__Tryndamere)))
//		{
//			CDodge::GetInstance()->DodgeTRYNDAMERE_BASETIME(pBuff);
//		}
////==================================================================================//
////==================================================================================//
//	}
...
//VMProtectBegin("DLL_VM__CanCastSpell2");
	if(g_bCheckKeyPress || g_bNormalAttackHero || g_bPressingKey_S)
	{
		if(STT_SKILL == 0 && /*SLMHTClientData::GetInstance()->GetCBComboUseQ()*/ pLang->isComboUseQ == false) return 32;
		if(STT_SKILL == 1 && /*SLMHTClientData::GetInstance()->GetCBComboUseW()*/ pLang->isComboUseW == false) return 32;
		if(STT_SKILL == 2 && /*SLMHTClientData::GetInstance()->GetCBComboUseE()*/ pLang->isComboUseE == false) return 32;
		if(STT_SKILL == 3 && /*SLMHTClientData::GetInstance()->GetCBComboUseR()*/ pLang->isComboUseR == false) return 32;
	}
	if(g_bHarass)
	{
		if(STT_SKILL == 0 && pLang->isHarassUseQ /*SLMHTClientData::GetInstance()->GetCBHarassUseQ()*/ == false) return 32;
		if(STT_SKILL == 1 && pLang->isHarassUseW /*SLMHTClientData::GetInstance()->GetCBHarassUseW()*/ == false) return 32;
		if(STT_SKILL == 2 && pLang->isHarassUseE /*SLMHTClientData::GetInstance()->GetCBHarassUseE()*/ == false) return 32;
		if(STT_SKILL == 3 && pLang->isHarassUseR /*SLMHTClientData::GetInstance()->GetCBHarassUseR()*/ == false) return 32;
	}
	if(g_bClearLane)
	{
		if(STT_SKILL == 0 && pLang->isLaneClearUseQ /*SLMHTClientData::GetInstance()->GetCBClearLaneUseQ()*/ == false) return 32;
		if(STT_SKILL == 1 && pLang->isLaneClearUseW /*SLMHTClientData::GetInstance()->GetCBClearLaneUseW()*/ == false) return 32;
		if(STT_SKILL == 2 && pLang->isLaneClearUseE /*SLMHTClientData::GetInstance()->GetCBClearLaneUseE()*/ == false) return 32;
		if(STT_SKILL == 3 && pLang->isLaneClearUseR /*SLMHTClientData::GetInstance()->GetCBClearLaneUseR()*/ == false) return 32;
	}
...
bool DO_FLASH(KPos* maxDest)
{
	...
	for (int i = 0; i < 20; ++i)
	{
		...
		if (CheckName(pKSkill_2GetName->GetSkillName(),  (sz__SummonerFlash)) && CheckName( (sz__SummonerFlash), pKSkill_2GetName->GetSkillName()))
		{
			...
			//if		( i == _Q && (float)GetTickCount() - Tick_doQ < MaxSkillDelay_1 ) return false;	
			//else if ( i == _W && (float)GetTickCount() - Tick_doW < MaxSkillDelay_1 ) return false;
			//else if ( i == _E && (float)GetTickCount() - Tick_doE < MaxSkillDelay_1 ) return false;
			//else if ( i == _R && (float)GetTickCount() - Tick_doR < MaxSkillDelay_1 ) return false;
			//else if ( i >  _R && (float)GetTickCount() - Tick_doOtherSkill < MaxSkillDelay_2 ) return false;
			
			////if (is_China == false)
			{
				if		( i == _Q && (float)GetTickCount() - Tick_doQ < pLang->_Delay_Qskill ) return false;	
				else if ( i == _W && (float)GetTickCount() - Tick_doW < pLang->_Delay_Wskill ) return false;
				else if ( i == _E && (float)GetTickCount() - Tick_doE < pLang->_Delay_Eskill ) return false;
				else if ( i == _R && (float)GetTickCount() - Tick_doR < pLang->_Delay_Rskill ) return false;
				else if ( i >  _R && (float)GetTickCount() - Tick_doOtherSkill < MaxSkillDelay_2 ) return false;
			}
			...
			DWORD retnAddress	  = OFF_FUCKING_CAST - TEMP_ENTRY_POINT ;			//0xF66198;//0x3F8B7C; (9.22)
			DWORD retnInstruction = hModule + retnAddress; 							//retn instruction 
			__asm
			{
					push retnHere
			...
			if		( i == _Q ) Tick_doQ = (float)GetTickCount();	
			else if ( i == _W ) Tick_doW = (float)GetTickCount();
			else if ( i == _E ) Tick_doE = (float)GetTickCount();
			else if ( i == _R ) Tick_doR = (float)GetTickCount();
			else if ( i >  _R ) Tick_doOtherSkill = (float)GetTickCount();
...
{
	if(!pLang->isAutoIgniteOnOff) return false;
	KNpc*   p_Player = GetPlayer();
	if(!p_Player) return false;
	if (IS_DEAD(p_Player) == true) return false;
...
bool DO_IGNITE(KNpc* pEnemy)
{
	...
	for(int i = 4; i < 6; ++i)
	{
		...
		if(CheckName(pKSkill_2GetName->GetSkillName(), sz__SummonerDot) && CheckName(sz__SummonerDot, pKSkill_2GetName->GetSkillName()))
		{
			...
			DWORD retnAddress	  = OFF_FUCKING_CAST - TEMP_ENTRY_POINT ;			//0xF66198;//0x3F8B7C; (9.22)
			DWORD retnInstruction = G_Module + retnAddress; 						//retn instruction 
			__asm
			{
					push retnHere
					mov ecx, _ECX
...
float Get_AA_Delay(KNpc* pNPC)
{
	...
	if (nDelay < FLT_EPSILON) nDelay = 1;
...
{
	Vec3D result(0,0,0);
	if(!pNpc) return result;
	DWORD pMoving = GET_PMOVE_NEW(pNpc);
	if(!pMoving) return result;
	stMOVE *unitPos = (stMOVE*) pMoving;
	result.x = unitPos->__strPosStart.x;
	result.y = unitPos->__strPosStart.y;
	result.z = unitPos->__strPosStart.z;
...
Vec3D GET_END_POS(KNpc* pNpc)
{
	...
	if(!pMoving) return result;
	stMOVE *unitPos = (stMOVE*) pMoving;
	result.x = unitPos->__strPosDest.x;
	result.y = unitPos->__strPosDest.y;
	result.z = unitPos->__strPosDest.z;

	return result;
}

//==================================================================================//
extern std::vector<DWORD> *badPointer_Unit;
bool IS_OBJ_DELETED(KNpc* Obj, char* szFuck)			//dung ham nay hay hon, neu ==true ---> doi tuong da~ bi xoa roi, ko ton tai nua
{
	...
	//if (badPointer_Unit && (int)badPointer_Unit->size() > 0)
	//{
	//	for (int i = 0; i < (int)badPointer_Unit->size(); ++i)
	//	{
	//		if ((DWORD)Obj == badPointer_Unit->at(i))
	//		{
	//			//__oMsg("--------------BAD-POINTER-IN-LISTTTT: %x [i=%d][size=%d]", Obj, i , (int)badPointer_Unit->size());
	//			//SEND_TEXT_SYSTEM("OBJ.DELETED.BADPOINTER-2");
	//			return true;
	//		}
	//	}
	//}

	//if (IsBadReadPtr(Obj, 750))
	//{
	//	badPointer_Unit->push_back((DWORD)Obj);

	//	__oMsg("[%s]--------------BAD-POINTER: %x, size = %d", szFuck, Obj, (int)badPointer_Unit->size());
	//	SEND_TEXT_SYSTEM("OBJ.DELETED.BADPOINTER-1");
	//	SEND_TEXT_SYSTEM(szFuck);
	//	SEND_TEXT_SYSTEM("------------------------");
	//	return true;
	//}
	...
	//__except(EXCEPTION_EXECUTE_HANDLER)
	//{
	//	SEND_TEXT_SYSTEM("Err: isObject.Deleted");
	//	return true;
	//}
...
bool IS_HERO(KNpc* pNpc)
{
	...
	//	for (int i = 0; i < (int)badPointer_Unit->size(); ++i)
	//	{
	//		if ((DWORD)pNpc == badPointer_Unit->at(i))
	//		{
	//			//SEND_TEXT_SYSTEM("IS.HERO.BADPOINTER-2");
	//			return false;
	//		}
	//	}
	//}

	//if (IsBadReadPtr(pNpc, 750))
	//{
	//	badPointer_Unit->push_back((DWORD)pNpc);
	//	SEND_TEXT_SYSTEM("IS.HERO.BADPOINTER-1");
	//	return false;
	//}
...
bool IS_TURRET(KNpc* pNpc)
{
	...
	//		if ((DWORD)pNpc == badPointer_Unit->at(i))
	//		{
	//			//SEND_TEXT_SYSTEM("IS.TURRET.BADPOINTER-2");
	//			return false;
	//		}
	//	}
	//}
	...
	//{
	//	badPointer_Unit->push_back((DWORD)pNpc);
	//	SEND_TEXT_SYSTEM("IS.TURRET.BADPOINTER-1");
	//	return false;
	//}
...
bool IS_NEXUS(KNpc* pNpc)
{
	...
	//		if ((DWORD)pNpc == badPointer_Unit->at(i))
	//		{
	//			//SEND_TEXT_SYSTEM("IS.NEXUS.BADPOINTER-2");
	//			return false;
	//		}
	//	}
	//}
	...
	//{
	//	badPointer_Unit->push_back((DWORD)pNpc);
	//	SEND_TEXT_SYSTEM("IS.NEXUS.BADPOINTER-1");
	//	return false;
	//}
...
bool IS_INHIBITOR(KNpc* pNpc)
{
	...
	//		if ((DWORD)pNpc == badPointer_Unit->at(i))
	//		{
	//			//SEND_TEXT_SYSTEM("IS.INHIBITOR.BADPOINTER-2");
	//			return false;
	//		}
	//	}
	//}
	...
	//{
	//	badPointer_Unit->push_back((DWORD)pNpc);
	//	SEND_TEXT_SYSTEM("IS.INHIBITOR.BADPOINTER-1");
	//	return false;
	//}
...
{
	return IsMoving(pNpc);



	if(!pNpc) return false;
	if(pNpc->nIS_MOVE_1 == 0 && pNpc->nIS_MOVE_2 != 0) return true;
	return false;
}
...
Vec3D GET_REAL_FACING_POS(KNpc* pNpc)
{
	...
	result.x = pNpc->currentPos().x + pNpc->facingPos.x;
	result.y = pNpc->currentPos().y + pNpc->facingPos.y;	//pNpc->facingPos.y luon = 0
	result.z = pNpc->currentPos().z + pNpc->facingPos.z;
	return result;
}
...
void Search_All_Unit_Around_NPC(KNpc* pNPC, float nRange, DWORD (&nResult)[300])
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(pObject->nIndex < 0x3FFFFCC8 && !IS_INHIBITOR(pObject) && !IS_NEXUS(pObject)) continue;
		{
...
bool IsDashing(KNpc* pNpc)
{
	...
	DWORD movNote = GET_PMOVE_NEW(pNpc);
	if(!movNote) return false;
	
	//DWORD tempMov  = *(DWORD*)(movNote + 4);
	//stMOVE *enemyPos = (stMOVE *)tempMov;
	stMOVE *enemyPos = (stMOVE*) movNote;
...
{
	//return 65.123f;
	float nResult = 65.12333f;
	if (pNpc == 0) return 1.22111f;

	if (IS_INHIBITOR(pNpc) || IS_NEXUS(pNpc)) return 99.669996f;	// 2 Object nay goi boundRadius la` Crash
...
bool IsValidObj(KNpc* pUnit)
{
	...
	int nCount_xxx = *(DWORD*)(dw0 + 0xC);
	if (!nCount_xxx) return false;

	for (int kk = 0; kk < nCount_xxx; ++kk)
	{
		DWORD dw3 = *(DWORD*)(dw1 + 4*kk);

		if (dw3 != 0 && dw3 == (DWORD)pUnit)
		{
			if (IS_OBJ_DELETED(pUnit) == false) return true;
		}
	}
...
bool IsValidTarget(KNpc* pUnit, float nRange, bool checkTeam, Vec3D* from)
{
	...
	__try
	{
		...
		if ((BYTE)pUnit->nIsTargetableToTeam != 4) return false;	//kha Quan Trong
		if (IS_DEAD(pUnit) == true)			       return false;

		//if (!IsUnitInList(pUnit)) return false;
		//if (IS_OBJ_DELETED(pUnit, "IsValidTarget")) return false;	// cho cai nay vao cung dc

		if (checkTeam && pUnit->nTeam_ID == pPlayer->nTeam_ID)	return false;

		//if((BYTE)pUnit->nIsTargetableToTeam != 4 /*|| IS_DEAD(pUnit) == true || (1 & (BYTE)pUnit->nIsInvulnerable) != 0*/)
		//{
		//	return false;
		//}

		if (pUnit->nIndex < 0x3FFFFCC8 && !IS_NEXUS(pUnit) && !IS_INHIBITOR(pUnit))
		{
			return false;
		}

		if (IsCanSee(pUnit) == false)
		{
			return false;
		}

		if(IS_HERO(pUnit) && IsInvulnerable(pUnit, DmgPhysical)) return false; //moi them

		//////Ko can check nua vi WardCorpse co nIsTargetableToTeam =3
		////if (CheckName("WardCorpse", pUnit->GetNameNpc()))
		////{
		////	return false;
		////}
		
		float nDistanceSquare = 0;
		if(from && from->IsValid()) nDistanceSquare = Distance(pUnit, *from, true);
		else						nDistanceSquare = Distance(pPlayer, pUnit, true);

		return nDistanceSquare < nRange*nRange;
	}
	__except(1)
	{
		//__oMsg("Err: IsValidTarget---4-------@@@@@---###================");
	}	
	return false;
...
bool  IsUnitInList(KNpc* pUnit)	//---- chi dung cho unitAttackAble
{
	...
	{
		DWORD dw3 = *(DWORD*)(dw1 + 4*i);
		if (dw3 != 0 && dw3 == (DWORD)pUnit) return true; 
	}
...
float GetDmg_TurretHitMinion(KNpc* turret, KNpc* minion)
{
	...
	if(minion->nOverrideCollisionRadius < 120)	//linh nho : Ranged && Melee
	{
		if((BYTE)minion->nCombatType == 1) return 0.45f*minion->nMaxHP;		// Melee : chinh xac 45%
		if((BYTE)minion->nCombatType == 2) return 0.68f*minion->nMaxHP;		// game ghi 70%, do dc 0.678
	}
	else	//linh to : Siege && Super
	{
		if((BYTE)minion->nCombatType == 1) return 0.05f*minion->nMaxHP;		// Super : game ghi 10% mau, nhung ko den
		if((BYTE)minion->nCombatType == 2) return 0.14f*minion->nMaxHP;		// Siege : tru ngoai 14%, tru giua 11%, tru can cu 8%
	}
	return 0;
}
...
{	
	...
	__try
	{
		...
		DWORD movNote = GET_PMOVE_NEW(unit);
		if(!movNote) return 0;
		//DWORD tempMov  = *(DWORD*)(movNote + 4);
		//DWORD pMove = tempMov;
		DWORD pMove = movNote;
	...
	{
		SEND_TEXT_SYSTEM("Path way");
		//__oMsg("CreatPathWays---");
	}
	return pResult;
}
...
{
	KNpc *pPlayer = GetPlayer();
	if(!pPlayer || !G_Module) return 0;
	int jCount = 0;
	DWORD tempUnit    = BASE_AttackableUnit     - TEMP_ENTRY_POINT + G_Module;	//dung vong lap kho xu li hon
	DWORD tempUnitMax = BASE_AttackableUnit_MAX - TEMP_ENTRY_POINT + G_Module;
	DWORD dw1 = *(DWORD*)tempUnit;
	DWORD dw2 = *(DWORD*)tempUnitMax;
	if (!dw1 || !dw2 ) return 0;
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(!IsValidTarget(minion, range)) continue;
		if (CheckName(sz__Minion, minion->GetNameNpc()) == false) continue;

		ProjectionInfo pProj = ProjectOn(minion->currentPos() ,pointstart, pointend);
		if (pProj.IsOnSegment && Distance(minion, pProj.SegmentPoint) <= width + GetBoundingRadius(minion)){
			jCount += 1;
		}		
	}
	return jCount;
...
{
	KNpc *pPlayer = GetPlayer();
	if(!pPlayer || !G_Module) return NULL;
	int aa = 2;
	KNpc* bestMinion = NULL;
	DWORD tempUnit    = BASE_AttackableUnit     - TEMP_ENTRY_POINT + G_Module;
	DWORD tempUnitMax = BASE_AttackableUnit_MAX - TEMP_ENTRY_POINT + G_Module;
	DWORD dw1 = *(DWORD*)tempUnit;
	DWORD dw2 = *(DWORD*)tempUnitMax;
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		KNpc *minion = (KNpc*)dw3;
		if(!IS_MINION(minion)) continue;
		if(IsMoving(minion)) continue;
		if(!IsValidTarget(minion, range)) continue;
		if (CheckName(sz__Minion, minion->GetNameNpc()) == false) continue;	
		if (CountMinionInLine(minion->currentPos(), Extend(pPlayer->currentPos(), minion->currentPos(), range), width, range) > aa)
		{
			aa = CountMinionInLine(minion->currentPos(), Extend(pPlayer->currentPos(), minion->currentPos(), range), width, range);
			bestMinion = minion;
		}
	}
...
KNpc * GetBestMininonCircleFarm(float width, float range)
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if(!IsValidTarget(minion, range)) continue;
		if (CheckName(sz__Minion, minion->GetNameNpc()) == false) continue;	
		if (Count_EnemyMinion_Around_NPC(minion, width) > aa)
		{
			aa = Count_EnemyMinion_Around_NPC(minion, width);
			bestMinion = minion;
		}
	}
...
{
	if(!pPlayer || !G_Module) return false;
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		ProjectionInfo pProj = ProjectOn(minion->currentPos() ,pPlayer->currentPos(), target->currentPos());
		if (pProj.IsOnSegment && Distance(minion, pProj.SegmentPoint) <= width + GetBoundingRadius(minion)){
			return true;;
		}
	}
...
bool AFacingB(KNpc* A, KNpc* B, float angle)
{
	...
	Vec3D rr1 = GET_REAL_FACING_POS(A);
	Vec3D DirectTarget = Extend(A->currentPos(), rr1, 400);

	//__oMsg("%f", AngleBetween3point(target->currentPos(), DirectTarget, pPlayer->currentPos()));

	return (AngleBetween3point(A->currentPos(), DirectTarget, B->currentPos()) <= angle/2);
}

bool Face2Face(KNpc* A, KNpc* B,float angle)
{
	if (AFacingB(A, B, angle) && AFacingB(B, A, angle))
	{
		return true;
	}
	return false;
