...
struct KNpc{//v6.07		
	int 		nTemp_1;
	DWORD		nIs_Nexus_KODUNGNUA;			// if(nIs_Nexus == offset_0x318C81C): phao dai			===>MA HOA ROI(SOLVED)
...
/**/int 		nTeam_ID;			//0x64:quan minh(blue), 0xC8:Doi phuong(red or purple), 0:ko thuoc ben nao
									//0x12C:Neutral
/**/int			nType_1_KODUNGNUA;	// =0x1401:Hero, 0x2401:Turret(thap ban), 0x20005: may cuc xxx, 0x00005: shop va trai linh(barracks)	===>MA HOA ROI(SOLVED)
...
/**/DWORD	    nIndex;				//0xFC	(4.294.967.295)
	...
	int         nDeleted;
	int 		nTemp_6_1_1[37];
	int			nCanSee_KODUNGNUA;			//0xDC: BYTE: 0:xa, 1:gan : nhin thay trong man hinh`		===>MA HOA ROI(NOT-YET-SOLVED)
	int 		nTemp_7[9];
...
/**/int			nIsDie_KODUNGNUA;				//0x118: BYTE: 0:song, 1:Die		"%s was killed by %s" - "%s was made Zombie by %s"	===>MA HOA ROI(SOLVED-TAM-THOI)
	KPos		CurrentPos;
...
/**/float		nGold;				//0x11AC
	...
	DWORD		_pMOVE_KODUNGNUA;				//===>MA HOA ROI(SOLVED)
...
//-------tam thoi dang tim den doan nay-------
	...
	//int 		nTemp_52[217];
	int 		nTemp_52[852];
	int			nBar_Source_KODUNGNUA;//3b2c(7.15)	//===>MA HOA ROI(SOLVED)->TAM THOI CHUA DUNG
	int 		nTemp_53[49];
	
	float		nCooldownExpiresBA_KODUNGNUA;			//===>MA HOA ROI(SOLVED): ko lay offset nay nua
	DWORD		_pNormalAttack;  //0x2ee4	//cu~

	int 		nTemp_53_1[26];
	BYTE		nIsMove_temp1;
	BYTE		nIsMove_KODUNGNUA;					//===>MA HOA ROI(SOLVED)
	BYTE		nIsMove_temp3;
	BYTE		nIsMove_temp4;
...
/**/DWORD		_pItem[39];			//gio no gop item voi skill la 1, vcc, pItem[0] = _pSkill[6]
	...
	bool		isJungle() {return (nTeam_ID == 0x12C) ? true : false;}
	...
	//Vec3D       startPos()	//ko dung` dc nua
	//{
	//	Vec3D rs;
	//	if(_pMOVE_KODUNGNUA)
	//	{
	//		DWORD tempMov  = *(DWORD*)(_pMOVE_KODUNGNUA + 4);
