...
void CIniManager::WriteIniFile()
{
	...
	{
		GetModuleFileName(hDll, g_szPath2DllFile_APP, MAX_PATH);
...
//----LastHit-Key-------------------------
	...
	SetChooseLine2(				m_IniFile->ReadBoolean( (sz__App_Information),  (sz__line2), true));
}

//-------------------------------------------------------------------------------------------------------
void CIniManager::WriteIniFile2(char* path_out)
{	
	//__oMsg("WriteIniFile__2");
	if (m_IniFile )
	{
		delete []m_IniFile ;
		m_IniFile  = 0;
	...
	strPath = strPath.substr(0, strPath.find_last_of(char(92)) + 1);
	strPath = strPath.append( (sz__config_AutoLoLs_ini));
	//__oMsg("[INI]duong dan den file-write: %s - %d", strPath.c_str(), GetAtkCurrentSelected());
